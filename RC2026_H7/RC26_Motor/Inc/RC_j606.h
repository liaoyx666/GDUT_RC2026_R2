...
public:
    ...
    void HybridControl(float p_des, float v_des, float tau_ff, float kp, float kd); // 混合控制
		float last_speed_filtered_ = 0.0f;
		float last_torque_filtered_ = 0.0f;

    float GetAngle() const { return total_angle_ * RAD_TO_DEG; }       // 获取当前角度(rad)
    float GetSpeed() const { return -speed_ * RAD_TO_DEG; }       // 获取当前速度(rad/s)
    ...
    bool IsEnabled() const { return is_enabled_; }    // 获取使能状态
		bool IsEnabledAck() const { return enable_ack_; }
...
private:
    ...
    // 多圈解算相关
    float last_angle_;    // 上一次的单圈角度(rad)
    int32_t round_count_; // 圈计数
    float total_angle_;   // 多圈累计角度(rad)
		
		uint32_t prev_raw_angle_;    // 上一次角度原始值
		
    int32_t rotation_count_;     // 圈数计数
		float angle_offset_;   // 上电零点偏移（rad）
		bool offset_initialized_; // 标志是否已初始化零点
