    ...
    : _A(A), _B(B), _predict_interval(predict_interval)
{
    ...
    this->updateQR(Q, R,Q_terminal);
}
template <int _states, int _commands, int _mpc_window>
void MpcSolver<_states, _commands, _mpc_window>::setConstraints(const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> &constraintMatrix,
...
template <int _states, int _commands, int _mpc_window>
void MpcSolver<_states, _commands, _mpc_window>::updateAB(
    const Eigen::Matrix<double, _states, _states> &A,
    const Eigen::Matrix<double, _states, _commands> &B)
{
    _A = A;
    ...
    // 重新计算离散化矩阵
    _A_bar = (Eigen::Matrix<double, _states, _states>::Identity() - 0.5 * _predict_interval * _A).inverse() * 
             (Eigen::Matrix<double, _states, _states>::Identity() + 0.5 * _predict_interval * _A);
...
void MpcSolver<_states, _commands, _mpc_window>::updateQR(const Eigen::Matrix<double, _states, _states> &Q, const Eigen::Matrix<double, _commands, _commands> &R,
                const Eigen::Matrix<double, _states, _states> &Q_terminal)
{
    _Q_terminal = Q_terminal;

    // 如果Q_terminal为零矩阵，则使用普通Q
    if (Q_terminal.isZero()) {
        // 设置所有预测步使用相同的Q
        for (int i = 0; i < _mpc_window; i++)
            _Q.block(i * _states, i * _states, _states, _states) = Q;
    } else {
        // 前N-1步使用普通Q，最后一步使用Q_terminal
        for (int i = 0; i < _mpc_window - 1; i++)
            _Q.block(i * _states, i * _states, _states, _states) = Q;
        _Q.block((_mpc_window - 1) * _states, (_mpc_window - 1) * _states, _states, _states) = Q_terminal;
    }
...
bool MpcSolver<_states, _commands, _mpc_window>::solveMPC(Eigen::Matrix<double, _commands, 1> &u)
{
    ...
    solver.data()->setNumberOfVariables(_commands*_mpc_window);
    solver.data()->setNumberOfConstraints(_constraintMatrix.rows());
    ...
    Eigen::SparseMatrix<double> constraintMatrixSparse = _constraintMatrix.sparseView();
    if (!solver.data()->setLinearConstraintsMatrix(constraintMatrixSparse))
        return false;
    if (!solver.data()->setLowerBound(_lowerBound))
        return false;
    if (!solver.data()->setUpperBound(_upperBound))
        return false;
