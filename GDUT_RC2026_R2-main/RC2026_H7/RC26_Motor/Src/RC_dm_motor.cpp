...
namespace motor
{
	...
	) : can::CanHandler(can_), tim::TimHandler(tim_), JointM(1.f, is_reset_pos_), use_mit(use_mit_), lp_td(fc, fs, zeta), 
		V_MAX(V_MAX_), V_MIN(-V_MAX_), T_MAX(T_MAX_), T_MIN(-T_MAX_)
	{
		id = id_;// 电机id
	...
	void DmMotor::CanHandler_Register()
	{
	...
	void DmMotor::Can_Tx_Process()
	{
		if (is_enable == true)
		{
			...
			float temp_target_torque = target_torque;// 加上前馈力矩
	...
	void DmMotor::Can_Rx_It_Process(uint32_t rx_id_, uint8_t *rx_data)
	{
	...
	void DmMotor::Set_K_Pos(float target_k_pos_)
	{
		if (use_mit == true)
		{
	...
	void DmMotor::Set_K_Spd(float target_k_spd_)
	{
		if (use_mit == true)
		{
