...
// ------------------ 正运动学 ------------------
    void ArmKinematics::forward(const JointAngles& angles, EndEffectorPos& end_pos) {
        ...
        T01 = buildDHTable(theta1, PI/2, 0.0f, BASE_HEIGHT, 0);
        T12 = buildDHTable(theta2, 0.0f, L1_LENGTH, 0.0f, THETA2_OFFSET);
        T23 = buildDHTable(theta3, 0.0f, L2_LENGTH, 0.0f, THETA3_OFFSET);
        T34 = buildDHTable(theta4, 0.0f, L3_LENGTH, 0.0f, THETA4_OFFSET);
        T04 = T01 * T12 * T23 * T34;
        end_pos.x = T04(0,3);
        end_pos.y = T04(1,3);
        end_pos.z = T04(2,3);
        end_pos.angle = normalizeAngle(theta2 +theta3+theta4+THETA4_OFFSET+THETA2_OFFSET+THETA3_OFFSET);
    }
    // ------------------ 逆运动学 ------------------
    bool ArmKinematics::inverse(const EndEffectorPos& target, JointAngles& result) {
        ...
        struct Cand { float q1, q2, q3, q4, score; } cands[2];
        int cand_count = 0;
        for (int i = 0; i < 2; ++i) {
            ...
            float Z = pz - L1_d;
            /* ---------- Wrist Center ---------- */
            float WR = R - L4 * cosf(pitch);
            float WZ = Z - L4    * sinf(pitch);

            float D = sqrtf(WR * WR + WZ * WZ);
            /* ---------- 检查可达性 ---------- */
            if (D > L2 + L3 || D < fabsf(L2 - L3)) continue;
            float alpha = acosf((L2 * L2 + D * D - L3 * L3) / (2.0f * L2 * D));
            ...
            float q3 = theta_v - theta_L2 - off3 ;
            q3 = normalizeAngle(q3);

            float q4 = pitch - ((q2 + off2) + (q3 + off3) + off4 );
            ...
            cands[cand_count++] = {q1, q2, q3, q4, score};
        }
        ...
        float rq4 = constrainValue(cands[best_idx].q4, THETA4_MIN, THETA4_MAX);
        result.theta1 = rq1;
        result.theta2 = rq2;
        result.theta3 = rq3;
        result.theta4 = rq4;
...
void Arm_task::executeMove()
    ...
    if (current_is_joint_space_)
    {
        ...
        for (int i = 0; i < ARM_JOINTS; i++)
        {
            ...
            cmd_velocities_.j[i] = (cmd_joints_.j[i] - last_cmd_joints_.j[i]) / dt* 60.0f / (2*PI);
        }
						last_cmd_joints_ = cmd_joints_;
    }
    else
    {
			//笛卡尔规划
        arm::ArmKinematics ik;
        arm::JointAngles q_sol;
        if (move_s_ < 1.0f)
        {
            vector3d::Vector3D next_pos = start_pose_.pos + (current_target_pose_.pos - start_pose_.pos) * move_s_;
            float next_pitch = start_pose_.pitch + (current_target_pose_.pitch - start_pose_.pitch) * move_s_;

            arm::EndEffectorPos next_effector{next_pos.x(), next_pos.y(), next_pos.z(), next_pitch};
            if (ik.inverse(next_effector, q_sol))
            {
                cmd_joints_ = {q_sol.theta1, q_sol.theta2, q_sol.theta3, q_sol.theta4};
            }
        }
        else {
            arm::EndEffectorPos tgt{current_target_pose_.pos.x(), current_target_pose_.pos.y(), 
                                    current_target_pose_.pos.z(), current_target_pose_.pitch};
            if (ik.inverse(tgt, q_sol)) {
                cmd_joints_ = {q_sol.theta1, q_sol.theta2, q_sol.theta3, q_sol.theta4};
            }
        }
				
        static arm::JointAngles last_q = cmd_joints_;
        for(int i = 0; i < 4; i++) {
            cmd_velocities_.j[i] = (cmd_joints_.j[i] - last_q.j[i]) / dt * 60.0f / (2*PI);
            last_q.j[i] = cmd_joints_.j[i];
        }
    }
}
...
void Arm_task::Tim_It_Process()
{
    ...
    // 4. 下发 cmd
    Arm_motor[0]->Set_Out_Mit_Pos(		cmd_joints_.j[0]);
		Arm_motor[0]->Set_Out_Mit_Rpm(cmd_velocities_.j[0]);
    Arm_motor[1]->Set_Out_Mit_Pos(		cmd_joints_.j[1]);
		Arm_motor[1]->Set_Out_Mit_Rpm(cmd_velocities_.j[1]);
		Arm_motor[2]->Set_Out_Mit_Pos(		cmd_joints_.j[2]);
		Arm_motor[2]->Set_Out_Mit_Rpm(cmd_velocities_.j[2]);
		Arm_motor[3]->Set_Out_Mit_Pos(		cmd_joints_.j[3]);
		Arm_motor[3]->Set_Out_Mit_Rpm(cmd_velocities_.j[3]);
}
